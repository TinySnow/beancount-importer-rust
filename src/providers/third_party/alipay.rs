//! 支付宝账单导入适配器。
//!
//! 该模块实现第三方支付供应商 `alipay`，并复用共享现金流转换器完成：
//! - 规则匹配与忽略判定；
//! - 收支方向推断与双分录构建；
//! - 订单号和扩展字段元数据附加。
//!
...
//!     },
//!     providers::third_party::alipay::AlipayProvider,
//! };
//!
//! let provider = AlipayProvider;
//! assert_eq!(provider.name(), "alipay");
//! assert_eq!(provider.description(), "Alipay statement importer");
//!
...
impl Provider for AlipayProvider {
    ...
    fn transform(
        ...
        transform_cashflow_record(self.name(), self.display_name(), ALIPAY_OPTIONS, record, rule_engine, config)
    }
}
